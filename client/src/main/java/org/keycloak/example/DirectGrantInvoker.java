...
public class DirectGrantInvoker
{
  ...
  public KeycloakToken keycloakAuthenticate() throws IOException, VerificationException
  {
    try (CloseableHttpClient client = HttpClientBuilder.create().build())
    {
      ...
      final String json = checkResponse(client, post);
  ...
  public void logout(final KeycloakToken keycloakToken) throws IOException
  {
    try (CloseableHttpClient client = HttpClientBuilder.create().build())
    {
      final HttpPost httpGet = new HttpPost(KeycloakUriBuilder.fromUri(KEYCLOAK_ROOT)
      ...
      formparams.add(new BasicNameValuePair(OAuth2Constants.GRANT_TYPE, "password"));
      UrlEncodedFormEntity form = new UrlEncodedFormEntity(formparams, "UTF-8");
      httpGet.setEntity(form);

      final String response = checkResponse(client, httpGet);
